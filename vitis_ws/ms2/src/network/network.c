...
void unpack(int32_t *packed_weights, param_t *unpacked_weights)
{
	unpacked_weights[0] = (*packed_weights >> 24) & 0xFF;
	unpacked_weights[1] = (*packed_weights >> 16) & 0xFF;
	unpacked_weights[2] = (*packed_weights >> 8) & 0xFF;
	unpacked_weights[3] = *packed_weights & 0xFF;
}
    ...
    size_p weight_scale)
{
    ...
    param_t cached_weights[KERNEL_SIZE * KERNEL_SIZE * 4];
    int32_t packed_weights;
    param_t unpacked_weights[4];

    if (in_channels == 1 && out_channels <= 4) {
    	for (size_t i = 0; i < KERNEL_SIZE * KERNEL_SIZE * 4; i += 4) {
    		packed_weights = Xil_In32(weights + i);
    		unpack(&packed_weights, unpacked_weights);
    		for (size_t j = 0; j < 4; ++j)
    			cached_weights[i + j] = unpacked_weights[j];
    	}
    	for (size_t oc = 0; oc < out_channels; ++oc) {
    		convolve(cached_weights, in_channels, input, in_size,
    				output, out_size, oc,
					stride, padding,
		...
		for (size_t out_channel = 0; out_channel < out_channels; ++out_channel)
		{
			for (size_t oy = 0; oy < out_size; ++oy)
			{
				...
				{
					int32_t sum = 0;
					for (size_t in_channel = 0; in_channel < in_channels; in_channel += 4)
					{
						size_t weight_index = (out_channel * in_channels + in_channel) * KERNEL_SIZE * KERNEL_SIZE;
						for (size_t i = 0; i < KERNEL_SIZE * KERNEL_SIZE * 4; i += 4) {
							packed_weights = Xil_In32(weights + weight_index + i);
							unpack(&packed_weights, unpacked_weights);
							for (size_t j = 0; j < 4; ++j)
								cached_weights[i + j] = unpacked_weights[j];
						}
						for (size_t ic = 0; ic < 4; ++ic)
						{
							for (size_t ky = 0; ky < KERNEL_SIZE; ++ky)
							{
								for (size_t kx = 0; kx < KERNEL_SIZE; ++kx)
								{
									...
									size_t input_idx = ((size_t) (ic + in_channel) * in_size + in_y) * in_size + in_x;

									size_t weight_idx = ((size_t) ic * KERNEL_SIZE + ky) * KERNEL_SIZE + kx;

									sum += input[input_idx] * cached_weights[weight_idx];
								}
							}
						}
					}
					size_t output_idx = ((size_t) out_channel * out_size + oy) * out_size + ox;
					output[output_idx] = rescale(sum, act_scale_a, act_scale_b, weight_scale);
				}
			}
		}
    ...
    size_p weight_scale)
{
    ...
    for (size_t o = 0; o < output_size; ++o)
    {
        ...
        for (size_t i = 0; i < input_size; i += 4)
        {
        	...
        	packed_weights = Xil_In32(weights + weight_idx);
        	unpack(&packed_weights, unpacked_weights);
        	for (size_t j = 0; j < 4; ++j)
        	{
        		sum += input[i + j] * unpacked_weights[j];
        	}
        }
